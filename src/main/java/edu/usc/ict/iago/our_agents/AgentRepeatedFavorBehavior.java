...
public class AgentRepeatedFavorBehavior extends IAGOCoreBehavior implements BehaviorPolicy {
	...
	public AgentRepeatedFavorBehavior(LedgerBehavior lb) {
		super();
		this.lb = lb;
	}
	...
	protected void setUtils(AgentUtilsExtension utils) {
		...
		case 1: {
			...
			this.utils.LIE_THRESHOLD = 0.3;
			break;
		}
		case 2: {
			this.lb = LedgerBehavior.FAIR;
			this.utils.LIE_THRESHOLD = 0.45;
			break;
		}
		default:
			this.lb = LedgerBehavior.GREEDY;
			this.utils.LIE_THRESHOLD = 0.65;
			break;
		}
	}
	...
	public Offer getNextOffer(History history) {
		...
		// Assign ordering to the player based on perceived preferences. Ideally, they
		// would be opposite the agent's (integrative)
		...
		// Find most valued issue for player and VH (of the issues that have undeclared
		// items)
		...
		} else if (utils.getVerbalLedger() < 0) // we have favors to cash!
		{
			...
			propose.setItem(opponentFave, new int[] { allocated.getItem(opponentFave)[0] + free[opponentFave], 0,
					allocated.getItem(opponentFave)[2] });
			return propose;
		} else if (utils.getVerbalLedger() > 0) // we have favors to return!
		{
			if (lb == LedgerBehavior.GREEDY)// this agent doesn't care
			{
				// nothing, so continue
			} else if (lb == LedgerBehavior.FAIR)// this agent returns an entire column!
			{
				...
				utils.modifyOfferLedger(1);
				propose.setItem(userFave, new int[] { allocated.getItem(userFave)[0], 0,
						allocated.getItem(userFave)[2] + free[userFave] });
				return propose;
			} else // if (lb == LedgerBehavior.LIMITED)//this agent returns a single item. woo hoo
			{
				// return single item
				utils.modifyOfferLedger(1);
				propose.setItem(userFave, new int[] { allocated.getItem(userFave)[0], free[userFave] - 1,
		...
		if (userFave == -1 && opponentFave == -1) // We already have a full offer (no undecided items), try something
													// different
		{
		...
		} else if (userFave == opponentFave)// Both agent and player want the same issue most
		{
			...
			else // Otherwise just give the one item left to us, the agent
			{
				if (utils.adversaryRow == 0) {
					...
					propose.setItem(userFave, new int[] { allocated.getItem(userFave)[0] + 1, free[userFave] - 1,
	...
	protected int getAcceptMargin() {
		...
		switch (this.utils.getCurrentGame()) {
		case 1: {
			marginThreshold = game.getNumberIssues() - 2;
			marginThreshold += (int) (adverseEvents * 0.2);
			break;
		}
		case 2: {
			marginThreshold = game.getNumberIssues() - 1;
			marginThreshold += (int) (adverseEvents * 0.1);
			break;
		}
		default:
			marginThreshold = game.getNumberIssues();
			marginThreshold += (int) (adverseEvents * 0.05);
			break;
		}
		
		return Math.max(0, Math.min(marginThreshold, adverseEvents));// basic decaying will, starts with fair
	}
	...
	public boolean acceptOffer(Offer o) {
		...
		ServletUtils.log("acceptOffer method - Agent Value: " + myValue + ", Perceived Opponent Value: " + opponentValue
				+ ", Opponent BATNA: " + utils.adversaryBATNA, ServletUtils.DebugLevels.DEBUG);
		jointValue = myValue + opponentValue;
		return (myValue / jointValue > .6 & myValue >= utils.myPresentedBATNA)
				|| (!utils.conflictBATNA(utils.myPresentedBATNA, utils.adversaryBATNA)
						&& myValue >= utils.myPresentedBATNA); // The threshold of .6 is semi-arbitrary
	}
